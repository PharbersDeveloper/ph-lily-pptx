...
class phMergeCityWithDotImpl extends phMergeWithDot with phCommand {
    var path = ""
    val primary = 1 :: 2 :: Nil
    val seed = "#alfred#"
    ...
    override def exec(args: Any): Any = {
        /**
          * 1. 从storage中把数据拿出来
          */
        val rdd = phLyFactory.stssoo("city main frame").asInstanceOf[RDD[phLyCityDataSet]]
        ...
        val reval = rdd.keyBy(row => row.product_name + "===" + row.pack_des + "===" + row.tp)
            ...
            .map { iter =>
                ...
                if (iter._2._2 != None) {
                    ...
                    iter._2._1.dot = iter._2._1.value / addrate
                }
                iter._2._1
            }

        phLyFactory.stssoo = phLyFactory.stssoo + ("city frame dot" -> reval)
        phLyFactory.saveMidProcess("city frame dot", "hdfs:///test/mid/main-frame-with-dot/")
        Some(reval)
    }
}
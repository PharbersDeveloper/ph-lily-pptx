...
import com.pharbers.process.common.{phCommand, phLyMOVData, phLycalArray}
import com.pharbers.process.stm.step.pptx.slider.content.phReportTableCol
import org.apache.spark.rdd.RDD
import org.apache.spark.sql.DataFrame

class overViewGrowthTable extends phCommand with phReportTableCol {
    ...
    override def exec(args: Any): Any = {
        ...
        /**
          ...
          */
        val filter_func_dot: phLyMOVData => Boolean = phLyMOVData => {
            ???
        }
        val filter_func_rmb: phLyMOVData => Boolean = phLyMOVData => {
            phLyMOVData.tp == "LC-RMB"
        }
        val funcFileter: Map[String, phLyMOVData => Boolean] = Map("rmb" -> filter_func_rmb, "dot" -> filter_func_dot)
        ...
        val func_rmb: phLyMOVData => BigDecimal = phLyMOVData => {
            phLyMOVData.value
        }
        val func_dot: phLyMOVData => BigDecimal = phLyMOVData => {
            ???
        }
        val valueFuncMap: Map[String, phLyMOVData => BigDecimal] = Map("rmb" -> func_rmb, "dot" -> func_dot)
        val mktDisplayNameList = rddTemp.filter(x => x.date >= allTimelst.min)
                .filter(x => x.date <= allTimelst.max)
                .filter(x => funcFileter(primaryValueName)(x))
                .map { x =>
                ...
                .reduce((x, y) => {
                    ("all", x._2.zip(y._2).map(x => x._1 + x._2))
                })._2

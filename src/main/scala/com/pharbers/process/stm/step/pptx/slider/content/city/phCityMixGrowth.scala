...
class phCityMixGrowth extends phCommand with phCityRankValue {
    override def exec(args: Any): Any = {
        ...
        val displayNameList = argsMap("displayNameList").asInstanceOf[List[String]]
        ...
        val func_Timeline: List[String] => List[String] = lst => lst.map { timeline =>
            List(timeline, timeline.dropRight(5) + (timeline.takeRight(5).take(2).toInt - 1).toString + timeline.takeRight(3))
        ...
        val mid_sum_Map = getAllDF(argsMap ++ Map("timelineList" -> timelineListTemp))
        ...
        val cityResultRDD = city_mid_sum.filter(x => cityList.contains(x._1._1)).map{x =>
            ...
            val somIncrease = valueList(2)(proDisplayNameIndex)/valueList(2)(mktDisplayNameIndex)-valueList(1)(proDisplayNameIndex)/valueList(1)(mktDisplayNameIndex)
            val proGrowth = valueList(2)(proDisplayNameIndex)/valueList(1)(proDisplayNameIndex)
            val growthIncrease = proGrowth - valueList(1)(proDisplayNameIndex)/valueList.head(proDisplayNameIndex)
            (x._1, List(somIncrease, growthIncrease, proGrowth))
        }
        val countryResultRDD = country_mid_sum.map{x =>
            val displayNameIndex = displayNameList.indexOf(x._1._2)
            val timelineIndex = allTimelineList.indexOf(x._1._2)
            val valueList = List.fill(displayNameIndex)(BigDecimal(0)) ::: List(x._2.result) ::: List.fill(displayNameSize-displayNameIndex-1)(BigDecimal(0))
            (x, List.fill(timelineIndex)(List.fill(displayNameSize)(BigDecimal(0))) ::: List(valueList) :::
                List.fill(alltimelineSize-timelineIndex-1)(List.fill(displayNameSize)(BigDecimal(0))))
